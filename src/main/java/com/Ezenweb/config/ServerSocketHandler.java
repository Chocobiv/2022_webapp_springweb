...
@Component  //해당 클래스를 beans[스프링 메모리]에 등록
public class ServerSocketHandler extends TextWebSocketHandler {
    //서버소켓

    private static List<WebSocketSession> list = new Vector<>();    //0. 접속자 명단 리스트
    ...
    public void afterConnectionClosed(WebSocketSession session, CloseStatus status) throws Exception {
        ...
        list.remove(session);//리스트 명단에 퇴장한 세션 제거
    }

    @Override   //3. 메시지 받았을 때
